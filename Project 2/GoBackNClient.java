...
public class GoBackNClient {
	...
	public static Fragment convertToFrag(DatagramPacket datagram){
		...
		Fragment fragOut = new Fragment(Arrays.copyOfRange(data, 18, data.length));
		fragOut.getmHeader().setCheckSum(Arrays.copyOfRange(data, 0, 16));
		fragOut.getmHeader().setSequenceID(data[16]);
		//Set fragment's end of Sequence value
		fragOut.getmHeader().setmEndOfSequence(data[17]);
	...
	public void sendAcknowledgements(){
		...
		byte[] ackBufferByteArray = null;
		ByteArrayOutputStream objbytetest = new ByteArrayOutputStream();
		ObjectOutputStream output;

		String[] newAckVector = new String[ackVector.size()];
		newAckVector = ackVector.toArray(newAckVector);
		if(trace){
			System.out.println(Arrays.toString(newAckVector) + " Transmitted");
		}
		try {
			output = new ObjectOutputStream(objbytetest);
			output.writeObject(newAckVector);
			output.flush();
			output.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
		ackBufferByteArray = objbytetest.toByteArray();
		try {
			objbytetest.flush();
			objbytetest.close();
		...
		DatagramPacket sendPacket = new DatagramPacket(ackBufferByteArray, ackBufferByteArray.length, serverIP, serverPortNumber);
		try {
